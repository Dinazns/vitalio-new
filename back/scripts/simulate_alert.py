...
Lance ce script depuis le dossier back/ avec le venv activé :
    ...
    python simulate_alert.py --metric temperature     # Hyperthermie (39.5°C) -> CRITICAL
    python simulate_alert.py --metric near_spo2       # SpO2 proche seuil (93 %) -> WARNING
    python simulate_alert.py --metric manual          # Alerte bouton patient -> URGENCY
...
from app.database import get_identity_db, get_medical_db, init_database
from app.services.alert_service import evaluate_measurement_alerts, create_manual_alert
...
PRESETS = {
    "spo2": {
        ...
        "temperature": 36.8,
        "label": "Hypoxémie (SpO2 = 85 %) -> CRITICAL",
    },
    "near_spo2": {
        "heart_rate": 76.0,
        "spo2": 93.0,        # proche du seuil 92 %, pas de breach
        "temperature": 36.7,
        "label": "SpO2 proche du seuil (93 %) -> WARNING",
    },
    "heart_rate_low": {
        "heart_rate": 38.0,  # seuil min = 50 bpm
        "spo2": 97.0,
        "temperature": 36.5,
        "label": "Bradycardie (FC = 38 bpm) -> CRITICAL",
    },
    "heart_rate_high": {
        "heart_rate": 135.0, # seuil max = 120 bpm
        "spo2": 96.0,
        "temperature": 37.2,
        "label": "Tachycardie (FC = 135 bpm) -> CRITICAL",
    },
    "temperature": {
        "heart_rate": 82.0,
        "spo2": 96.0,
        "temperature": 39.5, # seuil max = 38.0 °C
        "label": "Hyperthermie (Temp = 39.5 °C) -> CRITICAL",
    },
    "manual": {
        "label": "Alerte manuelle patient (bouton) -> URGENCY",
    },
}
...
def simulate(device_id: str, metric: str):
    ...
    if metric == "manual":
        ...
        if result["created"]:
            alert_doc = get_medical_db().alerts.find_one({"_id": ObjectId(result["alert_id"])})
            level = resolve_alert_severity_level(alert_doc or {})
            print(f"  [OK] Alerte manuelle creee  -> alert_id = {result['alert_id']}  severity_level = {level}")
        else:
    ...
    if triggered:
        ...
        print("  [i]  Aucune alerte CRITICAL declenchee (valeurs OK ou consecutivite non atteinte)")

    near_alerts = list(get_medical_db().alerts.find(
        {"device_id": device_id, "alert_source": "near_threshold", "status": "OPEN"},
        sort=[("updated_at", -1)],
        limit=3,
    ))
    for na in near_alerts:
        print(f"  [OK] Alerte WARNING (proche seuil) -> {na.get('metric')} severity_level={na.get('severity_level', 'WARNING')}")
