...
# Priorité : back/.env, puis back/app/.env, puis cwd.
# Évite de charger ../.env (souvent localhost) quand on lance l'API depuis un autre dossier.
for _env_path in (_BASE_DIR.parent / ".env", _BASE_DIR / ".env", Path.cwd() / ".env"):
...
# Field-level encryption (Fernet). Generate: python scripts/generate_field_encryption_key.py
