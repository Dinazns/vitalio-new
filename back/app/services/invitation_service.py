...
def send_device_confirmation_email(
    ...
    msg = MIMEMultipart("related")
    msg["Subject"] = "Confirmation d'enregistrement du dispositif VitalIO"
    msg["From"] = EMAIL_FROM
    msg["To"] = patient_email
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 500px; margin: 0 auto; padding: 20px;">
  ...
  <p style="text-align: center; margin: 24px 0;">
    <a href="{confirmation_url}" style="display: inline-block; padding: 14px 32px; background: #2563eb; color: #fff !important; text-decoration: none; border-radius: 8px; font-weight: bold;">Confirmer l'enregistrement</a>
  </p>
  <p>Si le bouton ne fonctionne pas, copiez ce lien :</p>
  <p style="word-break: break-all; font-size: 14px;"><a href="{confirmation_url}">{confirmation_url}</a></p>
...
"""
    ...
    img = MIMEImage(qr_bytes)
    img.add_header("Content-ID", "<qrcode>")
    img.add_header("Content-Disposition", "inline", filename="device-confirm-qr.png")
    msg.attach(img)
