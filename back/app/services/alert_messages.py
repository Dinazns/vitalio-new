...
def format_alert_for_doctor(alert: Dict[str, Any]) -> Dict[str, Any]:
    ...
    elif str(alert.get("alert_source") or "") == "near_threshold" or str(operator).startswith("near_"):
        label_map = {
            "heart_rate": "Fréquence cardiaque proche du seuil",
            "spo2": "SpO₂ proche du seuil",
            "temperature": "Température proche du seuil",
        }
        out["medical_label"] = label_map.get(metric, "Valeur proche du seuil")
        out["medical_description"] = (
            f"{label_map.get(metric, metric)} : {_v(value)} (seuil {_v(threshold)}). "
            "Surveillance renforcée recommandée ; pas encore de dépassement confirmé."
        )
    ...
    out["severity_level"] = resolve_alert_severity_level(alert)
    return out
...
def format_alert_for_caregiver(alert: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if metric == "manual":
        ...
        out["summary"] = "Une mesure approche la limite normale"
        out["lay_description"] = (
            f"Le capteur indique {_v(value)} (proche du seuil {_v(threshold)}). "
            "Ce n'est pas encore une alerte critique, mais une surveillance renforcée est conseillée."
        )
    elif metric == "heart_rate":
        if operator == "lt":
