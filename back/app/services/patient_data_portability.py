...
from app.services.field_encryption import decrypt_profile_fields
...
def build_patient_export(user_id_auth: str) -> Dict[str, Any]:
    ...
    for did in doctor_ids:
        raw = identity.users.find_one({"user_id_auth": did})
        doctors.append(decrypt_profile_fields(_serialize_user_doc(raw)))

    caregivers = []
    for cid in caregiver_ids:
        raw = identity.users.find_one({"user_id_auth": cid})
        caregivers.append(decrypt_profile_fields(_serialize_user_doc(raw)))
