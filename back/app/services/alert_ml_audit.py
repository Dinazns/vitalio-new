...
def create_or_merge_alert_for_validated_ml(
    ...
    severity_level = severity_for_ml_anomaly_level(severity)
    ml_urgency = str(anomaly_doc.get("urgency") or "routine")
    if ml_urgency == "immediate":
        severity_level = highest_severity(severity_level, SEVERITY_URGENCY)
